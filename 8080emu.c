...
int emulate8080Op(state8080 *state) {
  ...
  switch (*opcode) {
    ...
    case 0x06:
      ...
      break;
    case 0x07: {
        uint8_t sevenbit = state->a & 0x80;
        sevenbit = sevenbit >> 7;
        state->a = state->a << 1;
        state->cc.cy = sevenbit;
        state->a = state->a ^ sevenbit;
      }
      break;
    case 0x08:
      break;
    case 0x09: {
        ...
        uint16_t answer = (uint16_t) hl + (uint16_t) bc;
        // if answer is zero, set zero bit.
        setZero(state, answer);
    ...
    case 0x0e:
      ...
      break;
    case 0x0f:{
        uint8_t zerobit = state->a & 0x1;
        zerobit = zerobit << 7;
        state->a = state->a >> 1;
        state->cc.cy = zerobit;
        state->a = state->a ^ zerobit;
      }
      break;
    case 0x10:
      unimplementedInstruction(state);
    ...
    case 0x19: {
      ...
      break;
    case 0x1a: {
        uint16_t answer = (state->d << 8) | state->e;
        state->a = state->memory[answer];
      }
      break;
    case 0x1b:
      unimplementedInstruction(state);
